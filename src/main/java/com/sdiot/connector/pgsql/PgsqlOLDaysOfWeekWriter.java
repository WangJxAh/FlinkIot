...
public class PgsqlOLDaysOfWeekWriter extends RichSinkFunction<Tuple3<String, String, Integer>> {
    private static final Logger logger = LoggerFactory.getLogger(PgsqlOLDaysOfWeekWriter.class);
...
//    private static final String dbPwd = "postgres";
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        statement.execute(SqlConstants.IU_OL_DAYS_OF_WEEK);
        statement.close();

        callableStatement = connection.prepareCall("{call f_iu_ol_days_of_week(?, ?, ?, ?)}");

...
public class PgsqlOLLenOfHalfHourWriter extends RichSinkFunction<Tuple3<String, String, Long>> {
    private static final Logger logger = LoggerFactory.getLogger(PgsqlOLLenOfHalfHourWriter.class);
...
//    private static final String dbPwd = "postgres";
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        statement.execute(SqlConstants.IU_OL_LENGTH_OF_HALF_HOUR);
        statement.close();

        callableStatement = connection.prepareCall("{call f_iu_ol_length_of_half_hour(?, ?, ?, ?)}");
